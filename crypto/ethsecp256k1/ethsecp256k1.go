...
func (pubKey PubKey) verifySignatureAsEIP712(msg, sig []byte) bool {
	eip712Bytes, err, typedData := eip712.GetEIP712BytesForMsg(msg)
	if err != nil {
		return false
	}
	verifySig := pubKey.verifySignatureECDSA(eip712Bytes, sig)
	if !verifySig {
		typedDataJson, _ := json.Marshal(typedData)
		logrus.Errorf(fmt.Sprintf("Unable to verify EIP-712 sig, typedData: %s\n", typedDataJson))
	}
	return verifySig
}
