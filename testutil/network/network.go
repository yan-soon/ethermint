	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		if i == 0 {
			if cfg.APIAddress != "" {
				...
				var err error
				apiListenAddr, _, _, err = testnetwork.FreeTCPAddr()
				if err != nil {
					return nil, err
				}
			}
			...
			if cfg.RPCAddress != "" {
				...
				rpcAddr, _, _, err := testnetwork.FreeTCPAddr()
				if err != nil {
					return nil, err
				}
			...
			if cfg.GRPCAddress != "" {
				...
				_, grpcPort, _, err := testnetwork.FreeTCPAddr()
				if err != nil {
					return nil, err
				}
			...
			if cfg.JSONRPCAddress != "" {
				...
				_, jsonRPCPort, _, err := testnetwork.FreeTCPAddr()
				if err != nil {
					return nil, err
				}
		...
		proxyAddr, _, _, err := testnetwork.FreeTCPAddr()
		if err != nil {
			return nil, err
		}
		tmCfg.ProxyApp = proxyAddr

		p2pAddr, _, _, err := testnetwork.FreeTCPAddr()
		if err != nil {
			return nil, err
		}
		...
		fee := sdk.NewCoins(sdk.NewCoin(cfg.BondDenom, sdkmath.NewInt(50000000000)))
