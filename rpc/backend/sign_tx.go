...
func (b *Backend) SendTransaction(args evmtypes.TransactionArgs) (common.Hash, error) {
	if b.clientCtx.Keyring == nil {
		b.logger.Error("keyring is empty")
		return common.Hash{}, fmt.Errorf("keyring is empty")
	}
...
func (b *Backend) Sign(address common.Address, data hexutil.Bytes) (hexutil.Bytes, error) {
	...
	signature, _, err := b.clientCtx.Keyring.SignByAddress(from, data, signing.SignMode_SIGN_MODE_UNSPECIFIED)
	if err != nil {
...
func (b *Backend) SignTypedData(address common.Address, typedData apitypes.TypedData) (hexutil.Bytes, error) {
	...
	// Sign the requested hash with the wallet
	signature, _, err := b.clientCtx.Keyring.SignByAddress(from, sigHash, signing.SignMode_SIGN_MODE_UNSPECIFIED)
	if err != nil {
		b.logger.Error("keyring.SignByAddress failed", "address", address.Hex())
		return nil, err
	}
